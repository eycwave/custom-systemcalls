...
//************************************************
// New Function Declarations: (SYSCALL_DEFINE2, SYSCALL_DEFINE1, SYSCALL_DEFINE3, SYSCALL_DEFINE1)
...
SYSCALL_DEFINE3(get_proc_state_string, pid_t, pid, void *, buf, size_t, size) {
    ...
    if (pid == 0) {
        state_string = task_state_array[task_state_index(current)];
        if (copy_to_user(buf, state_string, size)) {
            return -EFAULT;
        }
        return 0;
    }
    for_each_process(task) {
        if (task->pid == pid) {
            state_string = task_state_array[task_state_index(task)];
            if (copy_to_user(buf, state_string, size)) {
                return -EFAULT;
            }
            return 0;
...
SYSCALL_DEFINE1(name, char *, msg) {
    ...
    printk(KERN_INFO "name syscall called with \"%.256s\"\n", buf);
    return 0;
}
//************************************************
